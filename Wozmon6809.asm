...
nextchar:                   ; Wait for key press
            ...
            lda  kb_rptr     ; ; 00-0F keyboard read pointer
            ...
            leay  kb_buffer,x
            lda  ,y   ; get the character being pointed to by the keyboard pointer
...
keymap:
            fcb "????????????? `?" ; 00-0F
            fcb "?????q1???zsaw2?" ; 10-1F
            fcb "?cxde43?? vftr5?" ; 20-2F
            fcb "?nbhgy6???mju78?" ; 30-3F
            fcb "?,kio09??./l;p-?" ; 40-4F
            fcb "??'?[=????",$0a,"]?\??" ; 50-5F
            fcb "?????????1?47???" ; 60-6F
            fcb "0.2568",$1b,"??+3-*9??" ; 70-7F
            fcb "????????????????" ; 80-8F
            fcb "????????????????" ; 90-9F
            fcb "????????????????" ; A0-AF
            fcb "????????????????" ; B0-BF
